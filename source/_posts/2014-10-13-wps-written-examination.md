...
大题的第一道蛮好玩的，通过4个操作(输入S，全选，复制，粘贴）输出最多的字符。第二题运煤问题应该是从陈皓的博客里拿的，前阵子刚看过。第三题是让自己设计一个图片浏览的应用。第四题是问自己用过比较好的移动应用并说明好和不足的地方。最后的编程大题是MYSQL查表（这道题我全留白了,没需求也就一直没怎么用MYSQL...  
...
------------
关于技术面试。  
在人生第一次技术面之后我就觉得特激动（in 腾讯实习生招聘）。跟面试官聊的过程中发现了很多自己不熟悉的东西，也发现可以从另外一个角度看待一个既定事实。技术面试是个很好玩的过程，从那个时候开始我就这么认定了。  
